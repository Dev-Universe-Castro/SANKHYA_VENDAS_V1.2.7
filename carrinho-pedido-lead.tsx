...
export function CarrinhoPedidoLead({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent showCloseButton={false} className="max-w-full w-full h-full md:max-w-md md:max-h-[90vh] p-0 flex flex-col m-0 rounded-none md:rounded-2xl border-none shadow-xl overflow-hidden bg-white">
        {/* Header */}
              ...
              {itensArray.map((item, index) => (
                <div key={index} className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm hover:border-[#76BA1B]/30 hover:shadow-md transition-all group">
                  <div className="flex gap-4">
                    {/* Imagem do Produto */}
                    <div className="w-20 h-20 bg-slate-50 border border-slate-100 rounded-xl flex items-center justify-center flex-shrink-0 overflow-hidden">
                      <img
                        ...
                        onError={(e) => {
                          ...
                          if (parent) {
                            parent.innerHTML = `<div class="text-2xl text-slate-300 font-bold">${item.DESCRPROD?.charAt(0) || 'P'}</div>`
                          }
                        }}
                      />
                    </div>
