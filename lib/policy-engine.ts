...
export function calculateSpecificity(policy: PoliticaComercial, context: PolicyContext, verbose: boolean = false): number {
    let score = 0;
    const logs: any[] = [];
 ...
 */
export function resolveBestPolicy(policies: PoliticaComercial[], context: PolicyContext): PoliticaComercial | null {
    if (!policies || policies.length === 0) return null;

    console.group('🔍 [PolicyEngine] Resolving Best Policy');
    console.log('📋 Context:', JSON.stringify(context, null, 2));

    let bestPolicy: PoliticaComercial | null = null;
    let maxScore = -1;
